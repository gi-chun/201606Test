...
//		last_click_time = new Date().getTime();
	...
	$('#bUSR').click(function(){   			/*$a.navigate('BL/MBLMG0M0');*/	pop_error();    	});	//	설문조사
	$('#vSVCP').click(function(){   		/*$a.navigate('BL/MBLMG0M0');*/	pop_error();    	});	//	고객센터 치도보기
	$('#bFAQ').click(function(){
//		last_click_time = new Date().getTime();
...
//    	}
	...
	$('#vGDN').click(function(){   	
		...
		application.hasApp(gdnvId,function(rt){
			...
			if(rt){
				...
				//alert('경동 나비엔 App이 설치 되어있지 않습니다.');
				notiPop('안내','경동 나비엔 스마트톡 앱이 설치되어있지 않습니다.<br/>설치 화면으로 이동하시겠습니까?',true,false,{
					list : [{
						type : '',
						id : 'pCancelOK',
						name : 'Store 이동'
	...
	$('#bNOTI').click(function(){   		navigateBackToNaviGo('MNTMA0M0');    	});	//	공지사항
	$('#bMSG').click(function(){   			
		var rtMsg = {
				'bp' : bp,
				'ca' : ca,
...
//	});	//	회사소개
	...
	$('#vGSAJ').click(function(){   		
//		var bpInfo = JSON.parse(getAlopexCookie(mbp));
		if(bpInfo.bpname == '코원에너지서비스'){
    		application.startWebBrowser("http://www.skens.com/m/kr/koone/content/view.do?cate=service&m1=gassafety");
    	}else if(bpInfo.bpname == '부산도시가스'){
    		application.startWebBrowser("http://www.skens.com/m/kr/busan/content/view.do?cate=service&m1=gassafety");
    	}else if(bpInfo.bpname == '충청에너지서비스'){
    		application.startWebBrowser("http://www.skens.com/m/kr/cheongju/content/view.do?cate=service&m1=gassafety");
    	}else if(bpInfo.bpname == '영남에너지서비스'){
    		if(bpInfo.lsc.indexOf('E') > -1){
    			application.startWebBrowser("http://www.skens.com/m/kr/gumi/content/view.do?cate=service&m1=gassafety");
    		}else if(bpInfo.lsc.indexOf('F') > -1){
    			application.startWebBrowser("http://www.skens.com/m/kr/pohang/content/view.do?cate=service&m1=gassafety");
    		}
    	}else if(bpInfo.bpname == '강원도시가스'){
    		application.startWebBrowser("http://www.skens.com/m/kr/gangwon/content/view.do?cate=service&m1=gassafety");
    	}else if(bpInfo.bpname == '전북에너지서비스'){
    		application.startWebBrowser("http://www.skens.com/m/kr/jeonbuk/content/view.do?cate=service&m1=gassafety");
    	}else if(bpInfo.bpname == '전남도시가스'){
    		application.startWebBrowser("http://www.skens.com/m/kr/jeonnam/content/view.do?cate=service&m1=gassafety");        		
    	}else{
    		
    	}
	});	//	가스와 안전
	
	$('#vYGNB').click(function(){   	
//		var bpInfo = JSON.parse(getAlopexCookie(mbp));
		if(bpInfo.bpname == '코원에너지서비스'){
    		application.startWebBrowser("http://www.skens.com/m/kr/koone/content/view.do?cate=service&m1=supplyaskcharge&m2=fareguide");
    	}else if(bpInfo.bpname == '부산도시가스'){
    		application.startWebBrowser("http://www.skens.com/m/kr/busan/content/view.do?cate=service&m1=supplyaskcharge&m2=fareguide");
    	}else if(bpInfo.bpname == '충청에너지서비스'){
    		application.startWebBrowser("http://www.skens.com/m/kr/cheongju/rate/guide.do");
    	}else if(bpInfo.bpname == '영남에너지서비스'){
    		if(bpInfo.lsc.indexOf('E') > -1){
    			application.startWebBrowser("http://www.skens.com/m/kr/gumi/content/view.do?cate=service&m1=supplyaskcharge&m2=fareguide");
    		}else if(bpInfo.lsc.indexOf('F') > -1){
    			application.startWebBrowser("http://www.skens.com/m/kr/pohang/rate/guide.do");
    		}
    	}else if(bpInfo.bpname == '강원도시가스'){
    		application.startWebBrowser("http://www.skens.com/m/kr/gangwon/rate/guide.do");
    	}else if(bpInfo.bpname == '전북에너지서비스'){
    		application.startWebBrowser("http://www.skens.com/m/kr/jeonbuk/rate/guide.do");
    	}else if(bpInfo.bpname == '전남도시가스'){
    		application.startWebBrowser("http://www.skens.com/m/kr/jeonnam/rate/guide.do");        		
    	}else{
    		
    	}
	});	//	요금안내

//	$('#vNOTI').click(function(){   		navigateBackToNaviGo('MNTMA0M1');    	});	//	공지사항 상세
...
//	'	<h3>'+bpInfo.bpname+'<br />'+bpInfo.center+'</h3>'+
	'	<h3>'+(mbp.lsc.indexOf('B')==-1?'콜센터':'서비스센터')+'<br/><span>가스누출,전출입,요금문의</br>※이사 시 3일전 예약 필요</span></h3>'+
	'	<p class="tell"><span class="col_yellow">'+bpInfo.tel+'</span><span class="time">상담시간 09:00~18:00</span></p>'+
	'</div>';
...
//				"bp" : '00000000',	"ca" : '00000000'
	...
	httpSend("getMnpgInfo", param, mainSetting, function(errorCode, errorMessage){
		if (errorCode == "9999") {
...
function setMultiButton(){
	if(params.list.bpCaList.length < 2){
    	...
    	for(var i=0;i<params.list.bpCaList.length;i++){
    		console.log("########################");
    		console.log("ca ::"+ca);
    		console.log("params.list.bpCaList[i].ca ::"+params.list.bpCaList[i].ca);
    		console.log("result ::"+(ca == Number(params.list.bpCaList[i].ca)));
    		console.log("########################");
    		popCaListStr += '<li class="'+(ca == Number(params.list.bpCaList[i].ca)?'key_':'')+'num goMainInfo"><input type="hidden" value="'+params.list.bpCaList[i].regiogroup+','+params.list.bpCaList[i].bp+','+params.list.bpCaList[i].ca+','+params.list.bpCaList[i].sernr+','+params.list.bpCaList[i].anlage+'"/><a href="javascript:void(0);">'+Number(params.list.bpCaList[i].ca)+'</a></li>';
    	}
...
//		}else if(isOverChkDate(cb.e_adatsoll1,cb.s_adatsoll1)){
		}else if(isOverChkDateSpc(cb.e_adatsoll1,cb.s_adatsoll1,cb.adatsoll1,mbc.regiogroup)){
			chkstr += '5';
			console.log('#################################################1');
			selfChk = true;
	...
	if(!selfChk){
		...
		if(selfChkMulti){
			...
			var adatsollStr = '<div>'+
			'	<h3>자가검침</h3>'+
			'	<p class="notime">자가검침 대상이<br />아닙니다.</p>'+
			'</div>';
			...
			var adatsollStr = '<div>'+
			'	<h3>자가검침</h3>'+
			'	<p class="notime">자가검침 기간이<br />아닙니다.</p>'+
			'</div>';
			$('#cont1_left').html(adatsollStr);
		}
		chkstr += 'e';
		...
		console.log('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$7');
		var adatsollStr = '<div>'+
			'<h3 class="new">자가검침</h3>'+
			'<p class="dday"><span>'+getDDay(cb.e_adatsoll1)+'</span></p>'+
			'<p class="date">'+getMonthDayDOW(cb.s_adatsoll1)+' ~ '+getMonthDayDOW(cb.e_adatsoll1)+' <br /><span>자가검침 등록기간</span></p>'+
			'</div>';
		$('#cont1_left').html(adatsollStr);
		logf('자가검침 yes');
	}
...
//		console.log('@@@@2');
    	...
    	//var cont2Str = '<h3>마이 페이지</h3><ul><li>'+billMonth+'월 사용량  <span>'+cb.list.thsmonUsgQtyResult[0].ABRMENGE+'㎥</span></li>'+//
    	var cont2Str = '<h3>'+billMonth+'월 청구서</h3><ul>'+
    		'<li><span class="ltt">사용량  </span><span class="rtt">'+toMypageUSED(cb.list.thsmonUsgQtyResult[0].I_SD_ITEM_03)+' ㎥</span></li>'+
    		'<li><span class="ltt">청구량  </span><span class="rtt">'+toMypageUSED(cb.list.thsmonUsgQtyResult[0].ABRMENGE)+' MJ</span></li>'+
			'<li><span class="ltt">청구금액 </span><span class="rtt">'+chgNumberToMoney(cb.list.thsmonUsgQtyResult[0].ZBTRANX_99)+' 원</span></li>'+
			'<li><span class="ltt">납부기한 </span><span class="rtt">'+toDateAddDot(cb.list.thsmonUsgQtyResult[0].FAEDN)+'</span></li></ul>';
    	$('.cont2').html(cont2Str);

    	//최근추이
    	// cb.list.fyerUsgQtyResult
    	//var category = [ '4월','5월', '6월', '7월', '8월', '9월', '10월', '11월', '12월', '1월', '2월', '3월' ];
    	
//    	var datas1 = [117.0, 140.0, 160.0, 165.0, 0.0, 132.0, 117.0, 140.0, 160.0, 165.0, 120.0, {
//        	y:132.0,
//            color : '#ea002c'
//        }];
//    	var datas2 = [117.0, 140.0, 160.0, 165.0, 0.0, 132.0, 117.0, 140.0, 160.0, 165.0, 120.0, {
//			y:132.0,
//			marker:{
...
//		}];
    	...
    	var datas1 = getDatas1Month(cb.list.fyerUsgQtyResult,cb.list.thsmonUsgQtyResult[0].BUDAT);
//    	var datas1 = getDatas1(cb.list.fyerUsgQtyResult);
	...
	// 공지사항
	httpSend("getNtBdList", notiParam, function(Mcb){
		var str = '';
		for(var i=0;i<Mcb.list.ntBdList.length;i++){
			if(i > 2){// 3건만 표기
			}else{
				str += '<li ><a href="javascript:void(0);" class="viewNotice">'+replaceAll(Mcb.list.ntBdList[i].TITLE, '\'','&quot;');
...
//		notiPop('서비스 중지 안내','더 나은 서비스 제공을 위해<br/>아래 기간 동안 서비스가 중지 됩니다.<br/>불편을 드려 죄송합니다. 감사합니다.<br/>기간 : 9월 25일 20시 ~ 9월 29일 09시<br/>“풍요로운 한가위 보내세요”',true,false,{
//			list : [{
//				type : '',
//				id : 'pCancelVOK',
//				name : '다시보지 않음'
...
//	}
	...
	if(year==2016 && month==1 && (day==28)){
		...
		if(isDisablePopup != "true"){
			notiPop('시스템 점검 안내','더 나은 서비스 제공을 위하여<br/>아래 기간 동안 서비스가 중단 됩니다.<br/><br/>기간 : 1월 28일 22시 ~ 1월 29일 05시30분<br/><br/>불편을 드려 죄송합니다.<br/>감사합니다.<br/>',true,false,{
				list : [{	
					type : '',
					id : 'pCancelVOK',
					name : '다시보지 않음'
	...
	// 각 사별 공지사항 팝업
	var	isDisablePopup = getGlobalPreference("isDisableNoticePopup_"+cb.noticePopupSeq);
	if(isDisablePopup == "true"){
		var disableTime = getGlobalPreference("isDisableNoticePopup_time_"+cb.noticePopupSeq);
		var currentTime = new Date().getTime();
		if((currentTime - disableTime) > (1000*60*60*24*7)){
			putGlobalPreference("isDisableNoticePopup_"+cb.noticePopupSeq, "false");
			isDisablePopup = "false";
		}
	}
	if((isDisablePopup != "true") && (cb.noticePopupSeq != '') && (cb.noticePopupSeq != undefined)){
		...
		$('.noticePopupDontLookBack').click(function(){
			putGlobalPreference("isDisableNoticePopup_"+cb.noticePopupSeq, "true");
			putGlobalPreference("isDisableNoticePopup_time_"+cb.noticePopupSeq, new Date().getTime());
...
//		console.log('21 ing...2');
	...
	if((device.osName == 'Android' || device.osName == 'iOS')==false){ //웹이라면 (안드로이드, 아이폰이 아니라면)
		...
		for(var i=0;i<params.list.bpCaList.length;i++){
			if(params.list.bpCaList[i].controlGp != '0021'){
				...
				console.log("controlGp == 0021 [bp:"+params.list.bpCaList[i].bp+", ca:"+params.list.bpCaList[i].ca+", pn:"+params.list.bpCaList[i].mobile+"]");
			}
		}
...
//			setAlopexCookie('change21_20150917',"true");
	...
	//제3자 정보제공 동의 여부 체크
	/*
	if(cb.insuranceAt!='Y' && cb.insuranceAt!='N'){
		//동의 및 거부를 선택하지 않은 경우
		
		if(notiBp=='B000'){
			//담당회사가 코원인 경우에만 팝업 안내를 띄운다. 
			view_use_pop = $('.view_use_provide_info').bPopup({
				opacity: 0.6,
				speed: 300, 
			});
			$('#provide_info_popup_close').click(function(){
				view_use_pop.close();
			});
