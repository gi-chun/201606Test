...
//			,headerGroupWiderFirst : false // always show widest headerGroup at topmose
		...
		extractElement : function(elem) {
			...
			var grid = null;
			do {
				if(fromElement.getAttribute('data-alopexgrid-dataid') && !cell) {
					cell = fromElement;
				}
		...
		extractData : function(elem) {
			...
			if(data) {
				...
				var key = cell.getAttribute('data-alopexgrid-key') || (_getMappingFromColumnIndex(self, columnIndex)||{}).key;
...
//			self.state.columnWidthMap = self._calcCellWidth();
	...
	AlopexGrid.prototype.viewUpdate = function(viewoption) {
		...
		if(gridOption.height && !isNaN(Number(gridOption.height))) {
			...
			var titleHeight = self.$title.prop('offsetHeight');
			var pagerHeight = self.$pager.prop('offsetHeight');
				...
				if(mapping) {
					...
					if(mapping["styleclass"] && self.option.mappingStyleclassToHeader) {
						...
						if(typeof cl === "function") {
							cl = cl.call(self, null,null,mapping) || "";
						}
						if(typeof cl === "string") {
							className += (" " + cl);
						}
	...
	//		attr["id"] = self._cellIdFromData(data, mapping.columnIndex);
	...
	AlopexGrid.prototype._refreshBodyCells = function(forceRefresh){
		...
		for(var o=0,i=fromRenderedIndex,l=self.state.renderTargetDataIndexList.length;i<l;i++,o++) {
			...
			if(visibleHeight > (bodyCellEndY - bodyCellStartY) && !scrollyEnd) break;
			//if(visibleHeight > self.state.wrapperHeight*2) break;
		}
		...
		if(self.option.rowPadding) {
			var len = self.option.rowPadding;
			if(len === true) {
				len = self.option.paging.perPage;
			}
			for(var i=visibleDataIndex.length;i<len;i++) {
...
//			}
	...
	AlopexGrid.prototype.viewEventUpdate = function(data) {
		...
		if(self.option.cellInlineEdit) {
			$wrapper.on('dblclick' + eventns, '.bodycell', {self:self},function(e){
				...
				setTimeout(function(){
					...
					self._setCursorUnderPointer($newcell, pageX, pageY); 
					//var $input = $newcell.find('input,select,textarea');
					//$input.focus();
					//var val = $input.val();
					//$input.val('').val(val);
				},0);
		...
		if(self.option.useTabindexOnEditable === 'dynamic') {
			$document.on('click' + eventns + self.key + self.eventns,{self:self},function(e){
				var self = e.data.self;
	...
	AlopexGrid.prototype._scrollOffset = function(merge) {
		...
		offset["scrollTop"] = this.option.scroll ? self.state.lastScrollTop : 0;
		offset["scrollLeft"] = this.option.scroll ? self.state.lastScrollLeft : 0;
		return offset;
	};
...
//			var lookupItem = lookupTableItem[iconData._index.data];
	...
	AlopexGrid.prototype.dataEmpty = function(data) {
		...
		self._scrollerReset();
		self.calcAndRedraw();
		setTimeout(function(){self._triggerGridEvent('changed',{"api":"dataEmpty"});},0);
		//this.updateOption();
	};
