		...
		_blurHandler : function(e) {
			...
			if (el.options.pickertype === 'daily' || el.options.pickertype === 'weekly') {
				...
				if (!$.alopex.util.isValid(value)) {
					...
					date = new Date(value.substr(yearStartIdx, 4), Number(value.substr(monthStartIdx, 2)) - 1, value.substr(dayStartIdx, 2));
				} else {
					if (yearStartIdx > 0 && yearStartIdx > monthStartIdx) {
						...
						date = new Date(value.split(seperator)[1], Number(value.split(seperator)[2]) - 1, value.split(seperator)[0]);
					} else {
				...
				if (!$.alopex.util.isValid(value)) {
					...
					date = new Date(value.substr(yearStartIdx, 4), Number(value.substr(monthStartIdx, 2)) - 1);
				} else {
					if (yearStartIdx > 0) {
						date = new Date(value.split(seperator)[1], Number(value.split(seperator)[0]) - 1);
					} else {
						date = new Date(value.split(seperator)[0], Number(value.split(seperator)[1]) - 1);
					}
				}
			}
			//console.log(date, $.alopex.util.getDateString(date, format));
...
//		  });
		  ...
		  // value를 동적으로 넣었을때 maxdate, mindate 설정
//		  $(el._start).on('click', function() {
//		    var format = el.options.format;
//		    var seperator = format.replace('yyyy', '').replace('MM', '').replace('dd', '').substring(0,1);
//		    var endDate = $(el._end).val();
//		    if ($.alopex.util.isValid(endDate)) {
//		      $(el._start).update({
//		        'maxdate': $.alopex.widget.daterange._getDate(el, endDate, seperator)
//		      });
//		    }
//		    if ($.alopex.util.isValid(this.value)) {
//		      $(el._end).update({
//		        'mindate': $.alopex.widget.daterange._getDate(el, this.value, seperator)
//		      });
//		    }
//		  });
		},
...
//    	var text = $(el).find(':selected').text();
    	var text =el.querySelectorAll('option:checked')[0].innerHTML;
...
//      var text = $(e.currentTarget).find(':selected').text();
      var text =wrapper.querySelectorAll('option:checked')[0].innerHTML;
...
(function($) {
	...
	$.alopex.widget.dropdownbutton = $.alopex.inherit($.alopex.widget.object, {
		...
		defaultClassName: 'af-button af-dropdownbutton',
...
(function($) {
	...
	$.alopex.widget.groupbutton = $.alopex.inherit($.alopex.widget.group, {
		...
		defaultClassName: 'af-groupbutton',
...
(function($) {
	$.alopex.widget.table = $.alopex.inherit($.alopex.widget.object, {
		...
		scrollerClassName: 'af-table-scroller' + $.alopex.config.defaultComponentClass.table + '-wrapper',
...
(function($) {
	...
	$.alopex.widget.tree = $.alopex.inherit($.alopex.widget.object, {
		...
		setters : [ 'tree', 'createNode', 'expand', 'collapse', 'expandAll', 'collapseAll', 'deleteNode', 'editNode', 'setSelected', 'toggleCheckbox', 'toggleExpand', 'setDataSource' ],
	...
	$.extend($.alopex, {
		...
		page: function() {
			...
			$.each(inits, function(idx, arg) {
				...
				function runArg() {
					arg($a.pageId, $.extend(true, $a.parameters, $a.results, $a.popupdata));
				}
...
!function($) {
			...
			if(temp.length === 1 && temp[0] == ruleList[i]) { // rule is valid
				...
				if(ruleObject && ruleObject.thirdSegment && ruleObject.thirdSegment.indexOf('{') !== -1) {
					...
					for(var j=0; j<subruleList.length; j++) {
						...
						if(subruleObject) {
							result[subruleObject.thirdSegment] = {
					...
					result[ruleObject.thirdSegment] = {
						serviceName: ruleObject.firstSegment,
						controlName: ruleObject.secondSegment
					};
				}
	 ...
	 * @param attributeName
	 */
	$.alopex.setDataOnElements = function (elementList, data, attributeName) {
		if(!data) {return ;}
			...
			for(var i in rule) {
				...
				if(value != null && value != undefined) { // found value on data.
					...
					if($.alopex.DataBinder[controlName] && $.alopex.DataBinder[controlName].render) {
						...
						$.alopex.DataBinder[controlName].render(element, i, value, data, rule[i]);
					}
				}
			}
		});
		...
		$.each(elementList, function(idx, element) {
			...
			for(var i in rule) {
				...
				if(controlName && $.alopex.DataBinder[controlName] && $.alopex.DataBinder[controlName].data) {
					var formatter = $(element).attr(FORMATTER_ATTRIBUTE);
					value = $.alopex.DataBinder[controlName].data(element, i, undefined, data, rule[i]);
					if(value != undefined && value != null) { // undefined나 null 인 경우가 아니면 그냥  키 생성.
						...
						$.alopex.util.setOnObject(data, i, value);
					}
				}
			}
		});
	...
	// NEW API!
	$.fn.setData = function(data, attributeName) {
		var candidates;
		attributeName = attributeName || BIND_ATTRIBUTE;
		candidates = this.findAll('[' + attributeName + ']').toArray();
		$.alopex.setDataOnElements(candidates, data, attributeName);
		return this;
	};
	...
	DataBinder["foreach"] = {
		...
		render: function(element, key, value, data, rule) {
			...
			if (!template) {
				var rule = $.alopex.BindParser.parseByControl($(element).attr(BIND_ATTRIBUTE));
				if(rule && rule.template && rule.template.id) {
					template = $('#' + rule.template.id).html();
				} else {
