...
//			}
		...
		init: function(el, options) {
			...
			$el.find('li > a').bind('click', function(e) {
				...
				if ($parent.is('li.af-expandable.Expandable')) {
					$parent.toggleClass('af-accordion-expand Expanded');
					if ($parent.hasClass('af-accordion-expand Expanded')) {
		...
		expandAll: function(el) {
			$(el).find('.af-accordion-sub').each(function() {
				var target = this;
				//var $li = $(target).closest('li').addClass('af-accordion-expand Expanded');
				$(target).css('display', '');
			});
		},
		...
		collapseAll: function(el) {
			$(el).find('.af-accordion-sub').each(function() {
				var target = this;
				//var $li = $(target).closest('li').removeClass('af-accordion-expand Expanded');
...
//		    });
		...
		_setDialogType: function(el, type) {
		  ...
		  if (type === 'blank' || type === 'close' || type === 'confirm' || type === 'okcancel') {
		    ...
		    if(el.toggle) {
		      ...
		      togglebtn.dialog = el;
		      $(togglebtn).attr('class', 'dialog_btn DialogBtn dialog_toggle');
		...
		open: function(el, obj, callbck) {
		  ...
		  if ($.alopex.util.isValid(obj)) {
		    ...
		    // 새롭게 옵션에 설정된 값 반영.
		    if ($.alopex.util.isValid(obj.type)) {
...
//    	var text = $(el).find(':selected').text();
    	...
    	if ($.alopex.util.isValid(selectedItem)) {
    		var text = selectedItem.innerHTML;
...
//      var text = $(e.currentTarget).find(':selected').text();
...
(function($) {
	...
	$.alopex.widget.dropdown = $.alopex.inherit($.alopex.widget.object, {
		...
		init: function(el, options) {
			...
			$el.on('click', 'li.af-focused.Focused', {
				element: el
...
//			el.totalpage = ($el.attr('data-totalpage')) ? parseInt($el.attr('data-totalpage'), 10) : Math.ceil(el.datalength/el.perpage);
			...
			var selectedPgae = parseInt($el.attr('data-selected'), 10) || parseInt($el.find('.selected').attr('data-page'), 10) || 1;
...
(function($) {
	$.alopex.widget.tabs = $.alopex.inherit($.alopex.widget.object, {
		...
		setters: ['tabs', 'setTabIndex','setTabByContent', 'setEnabled', 'setEnabledByContent', 'reload', 'clear', 'removeTab', 'setBeforeunload'],
...
//		},
		...
		init: function(el, options) {
			...
			if (eventAttr === 'hover' || eventAttr === 'hovering') {
				el.options.tabstrigger = 'hovering';
			}
				...
				for ( var i = 0; i < el.buttons.length; i++) {
					...
					if(el.options.removebutton) {
						...
						if (!tabButton.removeBtn) {
							...
							$(tabButton.removeBtn).attr('type', 'button').addClass('af-tabs-removebutton').appendTo(tabButton);
			...
			$.ajax({
				...
				complete: function(xhr, status) {
					...
					if (status === 'error') {
						...
						$(el).trigger('tabchange', [button.index]);
...
//					}
		...
		removeTab: function(el, index) {
			...
			if (el.buttons.length > 0 && index === el._currentTabIndex) {
				$(el).setTabIndex(0);
			}
		...
		_setRemoveBtnEvent: function(e) {
			...
			el.buttons.splice(index, 1);
			el.removebuttons.splice(index, 1);
			$(this.parentElement.content).remove();
	...
	$.alopex.widget.tree = $.alopex.inherit($.alopex.widget.object, {
		...
		_structure : function(ul) {
			...
			$(ul).find('> li')./*addClass('af-tree-node').*/each(function() {
				...
				if (isExpandable) {
					...
					if ($span.length === 0) {
						$('<span></span>').attr('class', 'af-tree-icon').attr('data-expand', 'false').insertBefore($(this).find('> a')[0]);
					}
		...
		_addCheckbox : function(el) {
			$(el).find('.af-tree-link').each(function() {
				var $checkbox = $(this).siblings('input.'+$a.config.defaultComponentClass.checkbox);
				if ($checkbox.length === 0) {
					$checkbox = $('<input type="checkbox"/>').insertBefore(this);
				}
				$checkbox.attr('data-type', 'checkbox').css('display', 'none').checkbox().bind('change click', $.alopex.widget.tree._checkboxHandler);
	...
	$.extend($.alopex, {
		...
		page: function() {
			...
			$.each(inits, function(idx, arg) {
				...
				function runArg() {
					arg($a.pageId, $.extend(true, $a.parameters, $a.results, $a.popupdata, $a.dialogdata));
					$a.dialogdata = null;
				}
...
!function($) {
	...
	function _GenerateId() {
		...
		var id = 'Alopex_Popup_' + parseInt(Math.random()*10E3);
	...
	function _DialogOpen(setting) {
		...
		if (setting.iframe) {
			...
			$dialog.load(setting.url, function() {
				$dialog.open(setting);
				// alopex dialog scroll 옵션과 중복되므로 삭제
				// if(setting.scroll){
...
//				}
				...
				var $activeDialogEl = $(window.parent.document.activeElement).closest('[data-type="dialog"]');
				...
				if(typeof selectors[i] == 'function') {
					...
					if($el.attr('data-alopexgrid')) {
						...
						if(griddata.key && griddata.list) {
							...
							if (!$.alopex.util.isValid(reference)) {
							  reference = {};
							}
							reference.list = {};
								...
								for(var i=0; i<selectors.length; i++) {
									if(typeof selectors[i] == 'function') {
										...
										if($el.attr('data-alopexgrid')) {
											...
											$(el).alopexGrid('dataSet', reference.list, (reference.list && reference.list.length > 0 && $.alopex.util.isValid(reference.currentPage))? {
...
//            req.originalResponse = res.responseText;
	...
	$.extend($.alopex.request, {
		...
		getGridData: function (grid) {
			...
			var $target = grid.jquery ? grid : $(grid);
			var list = AlopexGrid.trimData($target.alopexGrid('dataGet'));
...
!function($, window, document) {
	...
	$.alopex.services = function(svcList) {
		...
		for (var id in svcList) {
			if (!svcList.hasOwnProperty(id)) {
				return;
			}
			var setting = svcList[id];
			if(setting.trigger && setting.trigger.element && setting.trigger.event) {
				if(setting.trigger.event == 'load') {
					sendRequest(id);
				} else {
					$(setting.trigger.element).on(setting.trigger.event, (function(_id) {
						return function() {
							sendRequest(_id);
						};
					})(id));
	...
	function successCallback(data, textStatus, jqXHR) {
		...
		var defaultSetting = $.alopex.services.defaultSetting;
		
		var failConditions = $.merge(defaultSetting.response.failCondition, service.response.failCondition);
		for(var i=0; i<failConditions.length; i++) {
			var cond = failConditions[i];
			if(eval(cond[cond.op1] + cond[cond.op] + cond[cond.op2])) {
				// fail case
				failCallback();
				return ;
			}
		...
		if(service.after.processor && service.after.processor.length) {
			for(var i=0; i<service.after.processor.length; i++) {
				var source;
				if(service.after.processor[i].isCommon) {
					source = __services_common;
				} else {
					source = __services_page;
				}
				var after = source[service.after.processor[i].customFunction];
				if(after) {
					var result = after(data);
					if(result) {
						data = result;
					}
				}
		...
		// 공통 success callback
		if($.alopex.services.defaultSetting.success.callback && typeof __services_common[$.alopex.services.defaultSetting.success.callback] == 'function') {
			__services_common[$.alopex.services.defaultSetting.success.callback](data);
		}
		// 페이지 지정 success callback
		if(service.success.callback && typeof __services_page[service.success.callback] == 'function') { 
			__services_page[service.success.callback](data);
		}
		// 데이타 바인딩 처리.
		// databind 처리는 html에 작성된 'data-bind' 속성값에 따라 처리.
		var list = $.alopex.getCandidates(id, 'data-bind');
		$.alopex.setDataOnElements(list, data, 'data-bind', id);
	}
	
	
	function failCallback(errorCode, errorMsg) {
		var id = this;
		var service = $.alopex.services.list[id];
		var defaultSetting = $.alopex.services.defaultSetting;
		if(defaultSetting && defaultSetting.fail && defaultSetting.fail.callback && typeof __services_common[defaultSetting.fail.callback] == 'function') {
			__services_common[defaultSetting.fail.callback](errorCode, errorMsg);
		}
		if(service && service.fail && service.fail.callback && typeof __services_page[service.fail.callback] == 'function') {
			__services_page[service.fail.callback](errorCode, errorMsg);
		}
	}
	...
	function errorCallback(jqXHR, textStatus, errorThrown) {
		...
		var service = $.alopex.services.list[id];
		var defaultSetting = $.alopex.services.defaultSetting;
		if(defaultSetting && defaultSetting.error && defaultSetting.error.callback && typeof __services_common[defaultSetting.error.callback] == 'function') {
			__services_common[defaultSetting.error.callback](textStatus, errorThrown);
		}
		if(service && service.error && service.error.callback && typeof __services_page[service.error.callback] == 'function') {
			__services_page[service.error.callback](textStatus, errorThrown);
		}
	}
	...
	function sendRequest(id) {
		var service = $.alopex.services.list[id];
		var defaultSetting = $.alopex.services.defaultSetting;
		var setting = $.extend(true, {}, defaultSetting.request, service.request);
		setting.data = setting.parameter;
		if(setting.elements) {
			for(var i in setting.elements) {
				var tokens = i.split('.');
				$.alopex.util.setOnObject(setting.data, i, $(setting.elements[i]).getData({}, 'data-parameter')[tokens[tokens.length-1]]);
			}
		}
		if(setting.variables) {
			for(var i in setting.variables) {
				$.alopex.util.setOnObject(setting.data, i, setting.variables[i]);
			}
		}
...
//		setting.processData = false;		
		...
		setting.context = id;
		setting.success = successCallback;
		setting.error = errorCallback;
		delete setting.parameter;
		delete setting.variables;
		
		// handle pre-processes
		if(service.before.processor && service.before.processor.length) {
			for(var i=0; i<service.before.processor.length; i++) {
				if(service.before.processor[i].isCommon) {
					source = __services_common;
				} else {
					source = __services_page;
				}
				var before = source[service.before.processor[i].customFunction];
				if(before) {
					var result = before(setting.data);
					if(result) {
						setting.data = result;
					}
				}
	...
	$.alopex.services.defaultSetting = null;
	$.alopex.services.list = null;

	$.alopex.registerSetup('service', function(option) {
...
!function($) {
	...
	var FORMATTER_ATTRIBUTE = 'data-formatter';
	 ...
	 */
	function _setValueOnPath(object, path, value) {
		...
		for(var i=0; i<path_directory.length-1; i++) { /* iterate from 0 to length-1 */
			...
			if(!currentRef[dir]) {currentRef[dir] = {};}
	 ...
	 * @returns {boolean} 성공적으로 값을 셋했는지 여부
	 */
	function _getValueOnPath(object, path) {
		var path_directory = path.split('.');
		var currentRef = object;
	...
	$.alopex.util.setOnObject = _setValueOnPath;
	$.alopex.util.getFromObject = _getValueOnPath;
	...
	$.alopex.setDataOnElements = function (elementList, data, attributeName, serviceName) {
		...
		$.each(elementList, function(idx, element) {
			...
			for(var i=0; i<rule.length; i++) {
				if(serviceName!=rule[i].serviceName) {
					continue;
				}
				value = _getValueOnPath(data, rule[i].key);
				if(value != null && value != undefined) { // found value on data.
			...
			for(var i=0; i<rule.length; i++) {
				...
				if(controlName && $.alopex.DataBinder[controlName] && $.alopex.DataBinder[controlName].data) {
					...
					if(value != undefined && value != null) { // undefined나 null 인 경우가 아니면 그냥  키 생성.
						...
						$.alopex.util.setOnObject(data, rule[i].key, value);
					}
				}
			}
		});
	...
	$.alopex.getCandidates = function _getSettableElement(servicekey, attributeName) {
		var candidates = [];
		attributeName = attributeName || BIND_ATTRIBUTE;
		// setData 시점에는 가능한 candidate를 모두 입력해도 상관없다.
		$(document.body).findAll('[' + SERVICE_ATTRIBUTE +'="'+servicekey+ '"]').find('['+attributeName+']').each(function(idx, el) {
			if($(el).parents('[data-bind*="foreach"]').length>0) {
				return;
			}
			candidates.push(el);
		});
		$(document.body).findAll('[' + attributeName +'*="'+SERVICE_DELIMETER+servicekey+ '"]').each(function(idx, el) {
			if($(el).parents('[data-bind*="foreach"]').length>0) {
				return;
			}
			candidates.push(el);
	...
	$.alopex.getParamElements = function(servicekey, attributeName) {
		var candidates = [];
		attributeName = attributeName || PARAMETER_ATTRIBUTE;
		$(document.body).findAll('[' + SERVICE_ATTRIBUTE +'="'+servicekey+ '"]').each(function(idx, el) {
			candidates.concat(_getParsableElementInScope(el, attributeName));
		});
		$(document.body).findAll('[' + attributeName +'*="'+SERVICE_DELIMETER+servicekey+ '"]').each(function(idx, el) {
			if($.alopex.BindParser.isValidServiceData($(el).attr(attributeName), servicekey)) {
				candidates.push(el);
			}
		});
		return candidates; 
	...
	$.alopex.getDataByService = function(servicekey, attributeName) {
		var candidates = $.alopex.getParamElements(servicekey, attributeName);
		return $.alopex.getDataFromElements(candidates, attributeName);
	};
	
	
	// NEW API!
	$.fn.setData = function(data, attributeName, serviceName) {
		var candidates;
		attributeName = attributeName || BIND_ATTRIBUTE;
	...
	DataBinder["selectedOptions"] = {
		...
		render: function(element, key, value, data, rule) {
			if (value instanceof Array) {
				...
				$(element).val(value);
				if($(element).val() != value) { // select option을 서버로 부터 가져오기 때문에, 시점차가 생긴다.
					// selectedOption를 셋했는데, 제대로 바인드 안된경우, 체크해서 data 함수 호출 시 활용.
		...
		render: function(element, key, value, data, rule) {
			...
			for(var i=0; i<value.length; i++) {
				var newrow = $(template).clone()
								...
								.appendTo(element);
				$(newrow).setData(value[i], BIND_ATTRIBUTE, rule.serviceName);
			}
			
			event = document.createEvent('Event');
			event.initEvent('dataupdate', true, true);
			event.datalength = value.length;
			event.perpage = 0;
			event.totallength = 0;
